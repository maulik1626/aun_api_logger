...
6. **Dependency Version Pinning & Docs Sync**:
   - App integrations of `aun_api_logger` MUST use a pinned immutable reference (published version or git tag), never a moving branch reference like `ref: main`.
...
7. **Changelog Dependency Block (Mandatory for Every Version Bump)**:
   - Every `CHANGELOG.md` version entry MUST include a `### Dependencies` section with a dedicated yaml code block showing the exact `aun_api_logger` git `url` and pinned `ref` (git tag) for that version.
   - The `ref` in the dependency block MUST be the actual pushed git tag that contains that version's code changes.
   - The `README.md` dependency block MUST always show the latest version git tag.
